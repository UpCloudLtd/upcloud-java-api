...
//        api.getApiClient().setBasePath("http://localhost:8080/1.2");
        ...
        testFirewallRule
                .position(BigDecimal.valueOf(500))
    ...
    public void setUpEach() throws ApiException {
        ...
        FirewallRule firewallRule = api.createFirewallRule(
                serverId, new FirewallRuleRequest().firewallRule(
                        new FirewallRule()
                                .direction(FirewallRule.DirectionEnum.IN)
                                .family(AddressFamily.IPV4)
                                .protocol(FirewallRule.ProtocolEnum.TCP)
    ...
    public void createFirewallRuleTest() throws ApiException {
        ...
        FirewallRule createdRule = api.createFirewallRule(
                serverId,
                new FirewallRuleRequest().firewallRule(testFirewallRule)).getFirewallRule();
    ...
    public void deleteFirewallRuleTest() throws ApiException {
        ...
        FirewallRule createdRule = api.createFirewallRule(
                serverId,
                new FirewallRuleRequest().firewallRule(testFirewallRule)).getFirewallRule();
        final FirewallRule ruleForDelete = api.getFirewallRule(serverId, createdRule.getPosition()).getFirewallRule();
    ...
    public void serverServerIdFirewallRuleGetTest() throws ApiException {
        UUID serverId = testServer.getUuid();
        List<FirewallRule> firewallRules = api.serverServerIdFirewallRuleGet(serverId).getFirewallRules().getFirewallRule();

        assertTrue(firewallRules.stream().anyMatch(item ->
                item.getComment().equals("Common firewall rule")
        ));
    }
